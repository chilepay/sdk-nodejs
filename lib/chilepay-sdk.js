...
class Chilepay {
     ...
     *
     * @param {string?} params.body un texto libre en HTML con información extra de la transacción. Será mostrada
     * en la GUI de Chilepay al consultar los detalles de la transacción, tanto por el Comprador como por el Vendedor.
     * Considerar como si fuera el contenido de un email, aún cuando no es un email propiamente tal.
     * Máximo 140.000 carácteres (~ 140Kb )
     *
     ...
     * según la lógica de tu aplicación. Se mostrá en la GUI de Vendedor Chilepay. Máximo 40 carácteres.
     *
    ...
    /**
     *
     * @param {{notifyToken, status}} transaction el objeto devuelto por .getTransaction
     *
     * @return {string}
     */
    makeNotificationResponse(transaction) {
        const signature = crypto.createHash('sha256')
            .update(transaction.notifyToken + this.secretKey)
            .digest('base64');
        return JSON.stringify({ signature: signature, status: transaction.status });
    }
